-- Task 1: Find the most recent season (year) of the data.
SELECT MAX(year) AS most_recent_season
FROM performances;

-- Task 2: Find all players who hit exactly 17 home runs (HR) in the 1993 season.
...
-- Task 3: Use a scalar subquery to find players born in the most recent birth year.
SELECT 
  p.id,
  p.first_name,
  p.last_name,
...
-- Task 4: Find the year in which a player played the most games (G) across all records.
...
-- Task 5: What is the name of the player from the previous question (most games played)?
SELECT 
  p.first_name,
  p.last_name,
  pr.year,
...
-- Task 6: Retrieve team names where at least one player had more than 100 stolen bases (SB) in a season.
...
-- Task 7: Create a CTE that lists total hits per player per year. Then use the CTE in the main
